...
export default function ApplySubsidy() {
  ...
  const { id: subsidyIdParam } = useParams();

  const subsidyFromNav = location?.state?.subsidy || null;

  // Robust subsidy id (used for backend payload)
  const subsidyId = subsidyFromNav?.id || (subsidyIdParam ? Number(subsidyIdParam) : null);

  // Robust subsidy title (fixes "w Application Form" issue)
  const subsidyTitle =
    (subsidyFromNav?.title && subsidyFromNav.title.length > 2
      ? subsidyFromNav.title
      : subsidyFromNav?.name && subsidyFromNav.name.length > 2
      ? subsidyFromNav.name
      : 'Subsidy');
  ...
  function deriveTypesFromSubsidy(required) {
    ...
    return required
      .map(item => {
        ...
        if (item && item.value)
          return { value: item.value, label: item.label || prettifyLabel(item.value) };
        return null;
      })
      .filter(Boolean);
  }
  ...
  const [addModalTypes, setAddModalTypes] = useState(() =>
    deriveTypesFromSubsidy(subsidyFromNav?.documents_required || null)
  );
      ...
      if (profile) {
        const populatedForm = {
          ...
          mobile: profile.mobile_number
            ? profile.mobile_number.toString().slice(-10)
            : '',
  ...
  const missingDocs = (subsidyFromNav?.documents_required || [])
    .filter(r => !presentTypes.has(typeof r === 'string' ? r : r.value || ''))
  ...
  const handleUpdateDocument = async e => {
    ...
    const validation = {
      number: validateDocNumber(docForm.number),
      file: docForm.file ? validateDocFile(docForm.file) : '',
    };
  ...
  const handleSubmitApplication = async () => {
    if (!subsidyId) {
      alert('Invalid subsidy. Please open the subsidy from the list again and try applying.');
      return;
    }
    ...
    const mergedTypesBeforeUpload = new Set([
      ...documents.map(d => d.document_type),
      ...pendingUploads.map(p => p.document_type),
    ]);
    ...
    if (stillMissing.length) {
      alert(
        `You must provide required documents before submitting: ${stillMissing
          .map(prettifyLabel)
          .join(', ')}`
      );
      setStep(3);
    ...
    try {
      ...
      const payload = {
        subsidy_id: subsidyId, // <-- always a real id now
    ...
    <>
      <div className="fixed inset-0 bg-gray-100 bg-opacity-75 flex justify-center items-center p-2 sm:p-4 z-50 overflow-y-auto">
        ...
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-2xl w-full max-w-5xl p-4 sm:p-6 md:p-8 relative my-auto">
          ...
          {/* Fixed header title */}
          <h2 className="text-xl sm:text-2xl md:text-4xl font-extrabold text-gray-900 mb-2 text-center leading-tight pt-0 px-12">
            {`${subsidyTitle} Application Form`}
          </h2>

          <p className="text-center text-xs sm:text-sm text-gray-600 mb-6 md:mb-8">
            Step {step + 1} of 4
          </p>
          ...
          <form className="space-y-8" onSubmit={e => e.preventDefault()}>
            ...
            <div className={`${step === 0 ? '' : 'hidden'} w-full space-y-6`}>
              <h1 className="text-green-600 font-extrabold text-2xl md:text-3xl mb-6 pb-2 border-b-2 border-green-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="fullName">
                    Full Name
                  </label>
                  <input
                ...
                {/* Mobile */}
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="mobile">
                    Mobile Number
                  </label>
                  <input
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                ...
                {/* Aadhaar */}
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="aadhar">
                    Aadhar Number
                  </label>
                  <input
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  >
                    ...
                    {stateValue &&
                      Array.isArray(Data) &&
                      Data.find(s => s.state === stateValue)?.districts.map(
                        (districtData, index) => (
                ...
                <div className="flex flex-col">
                  ...
                  >
                    ...
                    {districtValue &&
                      ...
                      Data &&
                      Array.isArray(Data) &&
                      Data.find(s => s.state === stateValue)?.districts
                        .find(d => d.district === districtValue)
                        ?.subDistricts.map((subDistrictData, index) => (
                  ...
                  {errors.taluka ? (
                    <div className="text-red-500 text-sm mt-1">{errors.taluka}</div>
                  ) : null}
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  >
                    ...
                    {talukaValue &&
                      ...
                      Data.find(s => s.state === stateValue)?.districts
                        .find(d => d.district === districtValue)
                        ?.subDistricts.find(sd => sd.subDistrict === talukaValue)
                  ...
                  {errors.village ? (
                    <div className="text-red-500 text-sm mt-1">{errors.village}</div>
                  ) : null}
                </div>
              </div>

              <div className="flex flex-col">
                <label className="text-gray-700 font-semibold mb-2" htmlFor="address">
                  Address
                </label>
                <input
                  className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
            ...
            <div className={`${step === 1 ? '' : 'hidden'} w-full space-y-6`}>
              <h1 className="text-green-600 font-extrabold text-3xl mb-6 pb-2 border-b-2 border-green-200">
                Land Information
              </h1>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="landArea">
                    Land Area
                  </label>
                  <input
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                  ...
                  />
                  {errors.landArea ? (
                    <div className="text-red-500 text-sm mt-1">{errors.landArea}</div>
                  ) : null}
                </div>

                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="landAreaUnit">
                    Unit
                  </label>
                  <select
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                  ...
                  {errors.unit ? (
                    <div className="text-red-500 text-sm mt-1">{errors.unit}</div>
                  ) : null}
                </div>

                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="soilType">
                    Soil Type
                  </label>
                  <select
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                    ...
                    value={soilTypeValue}
                    onChange={e => {
                      setSoilTypeValue(e.target.value);
                      update('soilType', e.target.value);
                      validateField('soilType', e.target.value);
                    }}
                  >
                  ...
                  {errors.soilType ? (
                    <div className="text-red-500 text-sm mt-1">{errors.soilType}</div>
                  ) : null}
                </div>

                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="ownership">
                    Ownership
                  </label>
                  <select
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                    ...
                    onChange={e => {
                      ...
                      validateField('ownership', e.target.value);
                    }}
                  >
                  ...
                  {errors.ownership ? (
                    <div className="text-red-500 text-sm mt-1">{errors.ownership}</div>
                  ) : null}
                </div>
              </div>
            </div>
            ...
            <div className={`${step === 2 ? '' : 'hidden'} w-full space-y-6`}>
              ...
              <div className="space-y-6">
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="bankName">
                    Bank Name
                  </label>
                  <input
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                ...
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="ifscCode">
                    IFSC Code
                  </label>
                  <input
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                ...
                <div className="flex flex-col">
                  <label className="text-gray-700 font-semibold mb-2" htmlFor="accountNumber">
                    Account Number
                  </label>
                  <input
                    className="bg-gray-50 border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-green-600 focus:outline-none transition-all"
                  ...
                  />
                  {errors.bankAccount ? (
                    <div className="text-red-500 text-sm mt-1">{errors.bankAccount}</div>
                  ) : null}
                </div>
              </div>
            </div>
            ...
            <div className={`${step === 3 ? '' : 'hidden'} w-full space-y-6`}>
              ...
              {missingDocs.length > 0 && (
                <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-lg">
                  <strong className="font-bold">Missing required documents:</strong>{' '}
                  {missingDocs.map(prettifyLabel).join(', ')}
                </div>
              )}
                              ...
                              >
                                ...
                                <td className="py-2">
                                  {doc.title ||
                                    ...
                                    prettifyLabel(doc.document_type)}
                                </td>
                                ...
                                <td className="py-2">
                                  <div className="flex gap-2">
                                    <button
                                      ...
                                      className="px-2 py-1 border-2 border-green-600 text-green-600 rounded-md"
                                    >
                                      View
                                    </button>
                                    <button
                                      ...
                                      className="px-2 py-1 border-2 border-blue-600 text-blue-600 rounded-md"
                                    >
                                      Edit
                                    </button>
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleDelete(doc.id ?? doc.tempId)
                                      }
                                      className="px-2 py-1 border-2 border-red-600 text-red-600 rounded-md"
                                    >
                                      Delete
                                    </button>
                                  </div>
                    ...
                    <div>
                      ...
                      <div className="mb-4">
                        ...
                        <input
                          ...
                          onChange={e => {
                            setDocForm(f => ({ ...f, number: e.target.value }));
                            setDocErrors(prev => ({ ...prev, number: '' }));
                          }}
                      ...
                      <div className="mb-6">
                        ...
                        {docErrors.file && (
                          <p className="text-red-500 text-sm mt-1">{docErrors.file}</p>
                        )}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="mb-4">
                        ...
                        <input
                          ...
                          className={inputClass(docErrors.number)}
                          placeholder="e.g., ABC1234567"
                        />
                        {docErrors.number && (
                          <p className="text-red-500 text-sm mt-1">
                      ...
                      <div className="mb-6">
                        <label className="block text-gray-700 font-semibold mb-2">
                          Upload New Document (Optional, Max 5MB)
                        </label>
                        <input
                          type="file"
                          ref={fileInputRef}
                          onChange={handleDocFileChange}
                          className={inputClass(docErrors.file)}
                          accept=".pdf,.jpg,.jpeg,.png"
                        />
                        {docForm.file && !docErrors.file && (
                          <p className="text-sm text-green-600 mt-2">
            ...
            <div className="flex justify-between items-center pt-6 border-t border-gray-200 mt-8">
              ...
              {step < 3 ? (
                <button
                  type="button"
                  className="px-6 py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 transition-all duration-200 shadow-md hover:shadow-lg"
                  onClick={nextStep}
                >
                  Next →
                </button>
                ...
                <button
                  ...
                  className="px-8 py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
